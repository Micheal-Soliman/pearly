...
export default function ProductCard({ product, isFavorite, onToggleFavorite, onAddToCart, hrefQuery }: Props) {
  ...
  const isSqueeze = product.category === 'Squeeze';
  const needsShadeSelection = isBigBrush || isSqueeze;
  ...
  return (
    <>
          ...
          <div className="flex items-start justify-between gap-3 flex-1">
            <div className="flex flex-col h-full">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {(isLipgloss || needsShadeSelection) && (
      ...
      {/* Modal rendered outside motion.div to avoid transform clipping */}
      {needsShadeSelection && (
        <ShadesModal
          show={showShadeModal}
          ...
          onDone={() => {
            if (selectedShades.length === 1) {
              const shade = lipglossShadesForModal.find((s) => s.id === selectedShades[0]);
              const shadeName = shade?.name?.replace('Lipgloss - ', '') || selectedShades[0];
              const itemToAdd = {
                ...product,
                name: `${product.name} (Shade: ${shadeName})`,
                shadeId: selectedShades[0],
                quantity,
              };
          ...
          title={`Select Shade for ${product.name}`}
